...
import {generateScreen} from '../../generators-master/src';
...
export default {
    ...
    handler: async (params: ViewCommandConfig, context: Context) => {
        ...
        const tables: TableSchema[] = (await exportTables(context.request.bind(context), {withSystemTables: true})).filter((table) => !table.isSystem);
