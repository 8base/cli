...
export default {
  ...
  handler: async (params: MigrationStatusParams, context: Context) => {
    ...
    await executeDeploy(context, { mode: DeployModeType.migrations }, { customEnvironment: params.environment, nodeVersion: context?.projectConfig?.settings?.nodeVersion.toString() });
