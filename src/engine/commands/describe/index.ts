  ...
  handler: async (params: any, context: Context) => {
    ...
    if (resolvers.length > 0) {
      context.logger.info(
        table([
          RESOLVERS_HEADER,
          ...resolvers.map((resolver: any) => [resolver.name, resolver.description, resolver.gqlType]),
        ]),
      );
    } else {
    ...
    if (tasks.length > 0) {
      context.logger.info(
        table([TASKS_HEADER, ...tasks.map((task: any) => [task.name, task.description, task.scheduleExpression])]),
      );
    } else {
