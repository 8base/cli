...
const { E2E_SCAFFOLD_WORKSPACE_ID, E2E_SCAFFOLD_EMAIL, E2E_SCAFFOLD_PASSWORD } = process.env;
...
const generateScaffoldProject = async () => {
  try {
    ...
    const { stdout: createStdout } = await execa('node', ['../dist', 'generate', 'app', 'app-example'], {
      cwd: 'temp',
    });
    console.log(createStdout);

    await execa('node', ['../../dist', 'configure', `--workspaceId=${E2E_SCAFFOLD_WORKSPACE_ID}`], {
      cwd: 'temp/app-example',
    });
    const { stdout: brokersStdout } = await execa('node', ['../../dist', 'generate', 'scaffold', 'brokers', '--all'], {
      cwd: 'temp/app-example',
    });
    ...
    const { stdout: customersStdout } = await execa(
      'node',
      ['../../dist', 'generate', 'scaffold', 'customers', '--all'],
      { cwd: 'temp/app-example' },
    );
    ...
    const { stdout: listingStdout } = await execa('node', ['../../dist', 'generate', 'scaffold', 'listings', '--all'], {
      cwd: 'temp/app-example',
    });
    ...
    const { stdout: propertiesStdout } = await execa(
      'node',
      ['../../dist', 'generate', 'scaffold', 'properties', '--all'],
      { cwd: 'temp/app-example' },
    );
